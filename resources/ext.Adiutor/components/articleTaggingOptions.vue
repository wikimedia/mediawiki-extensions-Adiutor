...
<template>
	...
	<cdx-field>
		<table id="adiutor-options-props" width="100%">
			<caption>
				{{ $i18n( 'adiutor-tag-labels' ) }} <cdx-button
			...
			<tr v-for="( tag, tagIndex ) in label.tags" :key="'tag-' + tagIndex">
				<td>
					<cdx-text-input
						...
						aria-label="Value"></cdx-text-input>
				</td>
							...
							<tr v-for="( subitem, subitemIndex ) in tag.items" :key="'subitem-' + subitemIndex">
								<td>
									<cdx-text-input
										...
										aria-label="Subitem Value"></cdx-text-input>
								</td>
								<td>
								...
								<td>
									<cdx-text-input
										...
										style="min-width: 80px;"
										aria-label="Subitem parameter"></cdx-text-input>
								</td>
								<td>
									<cdx-text-input
										v-model="subitem.type"
										style="min-width: 50px;"
										aria-label="Subitem type"></cdx-text-input>
								</td>
								<td>
									<cdx-text-input
								...
								<td>
									<cdx-text-input
										...
										aria-label="Subitem help"></cdx-text-input>
								</td>
								<td style="text-align: right;">
									<cdx-button
										action="destructive"
...
module.exports = defineComponent( {
	...
	methods: {
		...
		generateTemplateOutput() {
			const output = this.useMultipleIssuesTemplate ? `{{${ this.multipleIssuesTemplate }|\n{{Cleanup|reason=Test}}\n{{Another tag}}\n}}` : '';
			return output;
		},
